#!/usr/bin/env bash
# Embed the OmniNode Share Extension into OmniNode.app/Contents/PlugIns.
...
codesign -d --entitlements - --xml "$PLUGINS/OmniNodeShareExtension.appex" 2>/dev/null | plutil -convert xml1 -o - - 2>/dev/null || true
