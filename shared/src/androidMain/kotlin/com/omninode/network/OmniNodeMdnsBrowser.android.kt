...
actual object OmniNodeMdnsBrowser {
    ...
    actual fun start(onPeerDiscovered: (host: String, port: Int, hintedDeviceId: String?) -> Unit) {
        ...
        val listener = object : NsdManager.DiscoveryListener {
            override fun onDiscoveryStarted(serviceType: String) {
                println("OmniNodeMdnsBrowser: discovery started")
            }

            override fun onServiceFound(serviceInfo: NsdServiceInfo) {
                if (!serviceInfo.serviceName.startsWith(OmniNodeMdns.SERVICE_NAME_PREFIX)) return
            ...
            override fun onStartDiscoveryFailed(serviceType: String, errorCode: Int) {
                println("OmniNodeMdnsBrowser: startDiscoveryFailed code=$errorCode")
            }
            override fun onStopDiscoveryFailed(serviceType: String, errorCode: Int) = Unit
        }
        discoveryListener = listener
        runCatching {
            manager.discoverServices(OmniNodeMdns.SERVICE_TYPE, NsdManager.PROTOCOL_DNS_SD, listener)
        }.onFailure { error ->
            println("OmniNodeMdnsBrowser: discoverServices failed — ${error.message}")
        }
    }
        ...
        manager.resolveService(
            ...
            object : NsdManager.ResolveListener {
                override fun onResolveFailed(info: NsdServiceInfo, errorCode: Int) {
                    println("OmniNodeMdnsBrowser: resolve failed code=$errorCode")
                }
    ...
    private fun resolveWithServiceInfoCallback(manager: NsdManager, serviceInfo: NsdServiceInfo) {
        val serviceCallback = object : NsdManager.ServiceInfoCallback {
            override fun onServiceInfoCallbackRegistrationFailed(errorCode: Int) {
                println("OmniNodeMdnsBrowser: callback registration failed code=$errorCode")
            }
    ...
    private fun deliverResolved(info: NsdServiceInfo) {
        ...
        val hintedId = OmniNodeMdns.deviceIdFromServiceName(info.serviceName)
