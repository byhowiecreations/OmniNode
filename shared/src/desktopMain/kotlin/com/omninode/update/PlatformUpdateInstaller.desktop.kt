...
actual object PlatformUpdateInstaller {
    actual fun updateCacheDirectory(): String {
        val dir = File(System.getProperty("java.io.tmpdir"), "OmniNodeUpdates")
        if (!dir.exists()) {
            dir.mkdirs()
        }
    ...
    actual fun installAndRelaunch(localFilePath: String, remoteVersion: String) {
        ...
        val scriptFile = File(updateCacheDirectory(), "omninode-apply-update.sh")
    ...
    private fun buildMacUpdateScript(assetPath: String, targetAppPath: String): String {
        ...
        return buildString {
            ...
            appendLine("TMP_DIR=\"${d}(mktemp -d /tmp/OmniNodeUpdate.XXXXXX)\"")
            ...
            appendLine("    echo \"OmniNode update: no .app found in DMG\" >&2")
            ...
            appendLine("  if [[ -z \"${d}SRC_APP\" ]]; then")
            appendLine("    echo \"OmniNode update: no .app found in ZIP\" >&2")
            appendLine("    exit 1")
            appendLine("  fi")
            appendLine("  ditto \"${d}SRC_APP\" \"${d}TARGET\"")
            appendLine("else")
            appendLine("  echo \"OmniNode update: unsupported asset ${d}ASSET\" >&2")
            appendLine("  exit 1")
            appendLine("fi")
            appendLine()
            appendLine("open -a OmniNode || open \"${d}TARGET\"")
        }
    }
