...
/**
 * Share Extension shell: write a pending job + open `fileapex://send?job=…`.
 *
...
object DesktopSendHandoff {
    ...
    fun sendJobUri(jobId: String): URI =
        URI("fileapex", /* authority */ "send", /* path */ null, /* query */ "job=$jobId", /* fragment */ null)
     ...
     * Safe to call multiple times; must run after [FileApexServices.init].
    ...
    private fun handleOAuthCallback(uri: URI): Boolean {
        ...
        if (scheme != "apex" && scheme != "fileapex") return false
